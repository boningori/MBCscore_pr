...
| リスク | 対処 |
|---|---|
| 更新直後、旧世代10件がすべて定期扱いになり3件まで落ちる | 落ちるのは最後の試合の終盤5分ぶんで、価値が低い。<br>**訂正（実装後のレビューで判明）:** 当初ここに「同じ起動で `startup` が1件入るので遡れる範囲は更新前より広がる」と書いたが、**更新の瞬間には成り立たない**。`startup` が写すのは現在の状態なので遡行距離はゼロで、最古の世代はむしろ10番目→3番目へ後退する。遡れる範囲が広がるのは翌日以降、`startup` と `gameEnd` が積み上がってから。実害は小さい（失うのは最終試合の終盤5分ぶん）が、更新直後だけは一時的に後退することを承知しておく |
