...
| 読み上げ用ラベル | `.sr-only` の「マイチーム」がパネルヘッダー・スコアボード・スタメン選択タブの3か所。相手側には付かない |
| 試合中のスタッツ表示（📊） | 選手別スタッツもマイチームが先。タブには虹を付けていないことも確認 |

（下2行は最終レビュー後の追加修正ぶんで、同じ dev サーバで再確認した）
...
置く先は3か所。記録画面のパネルヘッダー、スコアボードのラベル、そして**スタメン選択のタブ**である。

タブは当初この3つ目に入っていなかったが、最終レビュー（2026-09-20）で穴として指摘された。虹を足す理由として「位置の固定も視覚のみの手掛かりだから」と書いておきながら、位置の固定しか手掛かりが無いスタメン選択にこそ補完が無い、という筋の通らない状態になっていた。タブには読み上げラベルだけを足し、虹は付けない——タブは選択状態を色で表しており、そこに虹を重ねると「選択中かどうか」が読めなくなるため。
...
- **チームスタッツ比較（`TeamComparison`）・履歴の並びは変えない。** 紙のスコアシートと並べる場面では様式どおりの並びのほうが読みやすい。`TeamComparison` は履歴画面からも使われるため、並べ替えるならプロップで順序を受ける改修が要る。それに見合う要求がまだ無い。

  **ただし選手別スタッツ（`StatsPanel`）は並べ替える。** 設計時、この節は「試合後に落ち着いて見る画面だから」を除外の理由にしていたが、それは誤りだった。記録画面の📊ボタンは試合中に同じ画面をスタッツ表示へ切り替える経路で、`App.tsx` のコメント自身が「この画面を試合中に開く目的はほぼ『誰が何ファウル目か』の確認で、時間に追われている場面になる」と書いている。まさにこの修正が狙った状況であり、しかもトグルなので直前まで左にあったマイチームが同じ場所で右へ移っていた。最終レビューの指摘（2026-09-20）を受けて `StatsPanel` の2枚だけマイチーム順にした。同じブロックにある `TeamComparison` はそのまま。
