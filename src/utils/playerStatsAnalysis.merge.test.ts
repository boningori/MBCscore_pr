...
// 動くケース（誤字の訂正、コートネーム）は救えない。そこは利用者が統合して直す。
// 手動が常に自動判定に勝つ。
//
// 全角/半角スペースの混在は識別キー自体（generatePlayerKey）が吸収するように
// なった（Task 8）ため、ここでは統合が要る例として使わない。
...
describe('手動統合の反映', () => {
    it('統合すると1枚になり、通算・試合数・出場Qが合算される', () => {
        recordGame('タロウ', 4, 10, '2026-04-01'); // コートネームで登録していた時期
        ...
        expect(result[0].playerKey).toBe('佐藤太郎');
    ...
    it('試合別履歴が日付の新しい順に並んだまま合算される', () => {
        recordGame('タロウ', 4, 10, '2026-04-01');
        recordGame('佐藤 太郎', 4, 8, '2026-06-01');
        saveMergedPlayers(MY_TEAM.id, { 'タロウ': '佐藤 太郎' });
    ...
    it('統合したキーを非表示にすると一覧から消える', () => {
        ...
        recordGame('佐藤 太郎', 4, 8, '2026-06-01');
        saveMergedPlayers(MY_TEAM.id, { 'タロウ': '佐藤 太郎' });
        localStorage.setItem('minibasket-hidden-players', JSON.stringify({ [MY_TEAM.id]: ['佐藤太郎'] }));
    ...
    it('別チームの対応表は効かない', () => {
        recordGame('タロウ', 4, 10, '2026-04-01');
        recordGame('佐藤 太郎', 4, 8, '2026-06-01');
        saveMergedPlayers('別チーム', { 'タロウ': '佐藤 太郎' });
