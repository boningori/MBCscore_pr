...
export function RunningScoresheet({ game, gameName = '', date = '', onClose, onUpdateGameInfo, onEndTimeChange }: RunningScoresheetProps) {
    ...
    const [edges, setEdges] = useState(() => scrollEdges(null));
    const syncEdges = useCallback(() => {
        setEdges(prev => {
            const next = scrollEdges(scrollerRef.current);
            // 変わっていなければ同じオブジェクトを返す。横ドラッグ中は scroll が
            // 連続で飛ぶので、毎回新しい state を作ると描画が張り付く
            return prev.left === next.left && prev.right === next.right ? prev : next;
        });
    }, []);
    // 初回と、画面幅が変わったとき（回転・分割表示）に測り直す。
    // 送るのを待って測ると、一度も送らなかった人には目印が出ない
    useEffect(() => {
        syncEdges();
    ...
    return (
        <div className="running-scoresheet-container">
            ...
            {/*
              スコアシート本体はA4固定幅で、スマホでは枠に収まらない
              （実測375px: 枠343pxに対して本体794px）。横スクロールを
              この内側の要素に持たせ、影を出す枠はその外側に置く。
              中に置くと影も内容と一緒に流れて端に留まらない。
              出力が写すのは本体だけなので、枠の影は出力物に入らない
            */}
            <div
                className="scoresheet-scroll-frame"
                data-edge-left={edges.left}
                data-edge-right={edges.right}
            >
                <div className="scoresheet-scroller" ref={scrollerRef} onScroll={syncEdges}>
                    {/* スコアシート本体 */}
